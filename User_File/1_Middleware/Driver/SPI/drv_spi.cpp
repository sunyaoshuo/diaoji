...
/* LCD (SPI1) DMA 发送完成钩子: 弱定义, 由 bsp_lcd.cpp 覆盖, 解耦 drv_spi 与 LCD 驱动 */
__weak void BSP_LCD_SPI1_DMA_TxCpltHook(void) {}
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi)
{
    if (hspi->Instance == SPI1)
    {
        ...
        // LCD 非阻塞 DMA 发送完成回调 (DMA_Busy 为假时内部直接返回, 不影响其它 SPI1 用户)
        BSP_LCD_SPI1_DMA_TxCpltHook();
    }
    else if (hspi->Instance == SPI2)
    {
        if (SPI2_Manage_Object.Activate_GPIOx != nullptr)
